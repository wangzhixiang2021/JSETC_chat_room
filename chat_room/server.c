...
void User_server(int client_socket, Msg *msg)
{
	...
	while (j)
	{
		...
		case 8: //修改密码
			...
			break;
		case 886: //禁言某人
			server_bansb(client_socket, msg);
			break;
		case 888: //解除某人禁言
			server_nobansb(client_socket, msg);
			break;
		}
	}
...
//禁言某人
void server_bansb(int client_socket, Msg *msg)
{
	printf("管理员对%s禁言\n", msg->localname);
	int i;
	for (i = 0; i < NOW_MAX; i++)
...
//解除某人禁言
void server_nobansb(int client_socket, Msg *msg)
{
	printf("管理员对%s解除某禁言\n", msg->localname);
	int i;
	for (i = 0; i < NOW_MAX; i++)
